...
SearchQueue.prototype = {
  ...
  _filter: function(response) {
    ...
    const itemDescription = new Set();
    const results = [];
    response.hits.hits.forEach(doc => {
      if (!itemDescription.has(doc._source.description)) {
        itemDescription.add(doc._source.description);
        results.push(doc);
      }
    });
    return results;
  ...
  _process: function(snap) {
     ...
     if (this._assertValidSearch(key, dat)) {
        const parsedDat = JSON.parse(
            JSON.stringify(dat)
            .replace(/::/g, '.')
        );
      // Perform (a very simple) ElasticSearch query
      this.esc.search({
        index: parsedDat.index,
        type: parsedDat.type,
        body: parsedDat.query
      }, function(error, response) {
        if (error) {
          this._reply(key, {error: error.response, total: 0});
        } else {
          if (parsedDat.type === 'item') {
   ...
   _replyError: function(key, err) {
      this._send(key, { total: 0, error: JSON.parse(err) })
   },

   _send: function(key, data) {
      // setTimeout(() => this.inRef.child(key).remove(this._abortOnWriteError.bind(this)), 60000);
