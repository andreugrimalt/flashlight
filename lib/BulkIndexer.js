...
BulkIndexer.prototype = {
  ...
  insert: function(data) {
    const bulkBody = this.transform(data);
    this.esc.bulk({
      body: bulkBody
  ...
  transform: function(data) {
    const transformers = {
      'add': (item) => {
        ...
        if (this.config.type === 'order') {
          item.data = this._flattenOrderItem(item.data);
        }
        return [{
          index: {
  ...
  _flattenOrderItem: function(data) {
    data.items = data.items.map(item => {
      if (item.gtins) {
        item.gtins = Object.keys(item.gtins).map(gtinKey => {
          return {id: gtinKey};
        });
      }
      return item;
